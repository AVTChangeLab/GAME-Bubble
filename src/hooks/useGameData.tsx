...
export function useGameData(width: number, height: number, gameData:GameDataT) {
  let bubbles = gameData.totalBubbles
  const textContent = gameData.bubbles.map((val) => val.bubbleText ? val.bubbleText : 'noText')
  const bubbleAnswer = gameData.bubbles.map((val) => val.bubbleAnswer ? val.bubbleAnswer : false)
  const fontSize = gameData.bubbles.map((val) => val.fontSize ? val.fontSize : 0.25)
  ...
  const makeBubbles = (array: Vector3Object[]):bubbleT[] => {
    ...
    arrayCopy.forEach((pos,index) => {
      result = [...result, {position:pos,id:index, radius:radius[index], bubbleAnswer:bubbleAnswer[index], textContent:textContent[index], fontSize:fontSize[index]}]
    })
    return result
  }
  ...
  const assignPositions = () => {
    ...
    bubbles > shuffledPositions.length ? bubbles = shuffledPositions.length : shuffledPositions.length = bubbles
